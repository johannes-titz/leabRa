...
dim_lays <- list(c(1, 5), c(5, 4), c(5, 1))
...
inputs_plus <- net$create_inputs(which_layers = c(1, 3),
                              n_inputs = 5, prop_active = .3)

# remove the inputs for the output layer for minus phase
inputs_minus <- lapply(inputs_plus, function(x) {x[3] <- list(NULL); return(x)})
n_epochs <- 10
outs <- lapply(seq(n_epochs), function(x) net$learn_error_driven(inputs_minus, inputs_plus, lrate = seq(0.8, 0.2, length.out = n_epochs)))

# calc cosine

overall_cosine <- function(outs, inputs_plus, layer){
  outs_layer <- plyr::llply(outs[layer,], rbind)
  inputs_plus_layer <- lapply(inputs_plus, function(x) x[[layer]])
